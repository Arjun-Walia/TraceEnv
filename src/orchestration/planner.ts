  ...
  createPlan(projectRoot: string, workflow: WorkflowSpec): ExecutionPlan {
    ...
    return {
      ...
      resolvedSteps: orderedSteps.map((step, index) => ({
        ...
        cwd: resolveStepCwd(normalizedRoot, step.cwd),
...
function resolveStepCwd(projectRoot: string, stepCwd?: string): string {
  const resolved = path.resolve(projectRoot, stepCwd ?? '.');
  const relative = path.relative(projectRoot, resolved);

  const escapesProject = relative.startsWith('..') || path.isAbsolute(relative);
  if (escapesProject) {
    throw new ValidationError(`Workflow step cwd escapes project root: ${stepCwd ?? '.'}`);
  }
